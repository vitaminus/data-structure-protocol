...
async function runCase(benchCase: BenchCase, parallelism: number): Promise<BenchResult> {
...
const parallelismValues = (process.env.DSP_BENCH_PARALLELISM ?? "1,4")
  .split(",")
  .map((value) => Number(value.trim()))
  .filter((value) => Number.isFinite(value) && value > 0);
const requestedCases = new Set((process.env.DSP_BENCH_CASES ?? "tiny,medium,large").split(",").map((value) => value.trim()));
const benchCases = CASES.filter((benchCase) => requestedCases.has(benchCase.name.split("-")[0]));
