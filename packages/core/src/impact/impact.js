...
export function analyzeImpact(db, target) {
    ...
    const visited = new Set([targetUid, ...direct]);
    const queue = [...direct];
    const transitive = [];
    ...
    const suggestedFiles = [...new Set(allDependents.map((uid) => db.getEntity(uid)?.path).filter(Boolean))];
    return {
        target: targetUid,
